...
vlVoid CError::Set(const vlChar *cErrorMessage, vlBool bSystemError)
{
	...
	if(bSystemError)
	{
		...
		if(FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, uiLastError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&lpMessage, 0, NULL))
		{
